...
class AdminGroups(Helpful, Module):
    ...
    def create(self, config) -> list:
        ...
        query = """
        mutation CreateGroupMutation($payload: GroupCreateInput!) {
        ...
        # if 'data' in result and 'createGroup' in result['data']:
        #     return waring_if_needed(result['data']['createGroup'], self.primehub.stderr)
        return result

    @cmd(name='update', description='Update the group')
    ...
    def update(self, name: str, config: dict) -> list:
        ...
        query = """
        mutation UpdateGroupMutation($payload: GroupUpdateInput!, $where: GroupWhereUniqueInput!) {
          updateGroup(data: $payload, where: $where) {
            name
            displayName
          }
        }
    ...
    @cmd(name='delete', description='Delete a group by id', return_required=True)
    def delete(self, id):
        """
        ...
        :rtype dict
        :return the result of the deleted group
        """

        query = """
        mutation DeleteGroupMutation($where: GroupWhereUniqueInput!) {
        ...
        if 'data' in result and 'deleteGroup' in result['data']:
